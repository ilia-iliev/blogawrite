...
//! being looked at. Nothing here knows anything about Qt beyond how it counts, so all of
//! it can be asked directly.

use crate::text::utf16_at;
use std::sync::Arc;

/// One occurrence: the block it stands in, and where it starts and ends inside that
/// block, counted the way Qt counts.
    ...
    for (index, block) in blocks.iter().enumerate() {
        ...
        for (at, _) in block.char_indices() {
            ...
            found.push(Occurrence {
                block: index as i32,
                at: utf16_at(block, at) as i32,
                end: utf16_at(block, at + length) as i32,
            });
            skip_to = at + length;
        }
    }
...
mod tests {
    ...
    #[test]
    fn counts_the_way_qt_counts() {
        // The emoji is one character, two UTF-16 units and four bytes.
        let blocks = document("🙂 word");
        assert_eq!(occurrences(&blocks, "word"), [Occurrence { block: 0, at: 3, end: 7 }]);
    }
